...
|`* *` |user |have access to a Open button |load contact details from any data file I choose instead of following the default save location

|`* *` |user |have an auto-fill feature when typing in my tags |not have to remember all the tags that I ever wrote

|`* *` |user |have access to a Save As option |customize my save location instead of using the default one
...
* 2a. No such person was found.
+
[none]
** 2a1. AddressBook informs the user that no matching users were found.
+
Use case ends.
...
* 3a. The specified detail to change is invalid.
+
[none]
** 3a1. AddressBook shows an error message.
+
...
[none]
* 2a. The number of commands entered were less than the specified number of redo commands.
+
[none]
** 2a1. AddressBook redos all previously undone commands.
+
Use case resumes at step 3
...
.  The application should be intuitive for all users, regardless of technical ability.
.  The application's graphic user interface should display properly on monitors of all resolutions.
.  New features will have relevant tests created to root out possible bugs.
.  New features and changes are to be documented in the user and developer guides.
.  Saved data is to be backwards compatible from v1.0 onwards.
...
....
A feature set is a set of enhancements rolled out at each set interval
....
...
....
The Graphic User Interface (GUI) consists of the organised visual elements that users can see and interact with.
....
...
....
Bugs refer to errors and flaws in the implementation of features. It causes the application to behave unexpectedly or give incorrect outputs/outcomes.
....
...
....
An enhancement refers to an addition of a feature to the application with the intention of improving user experience. It is rolled out as part of a feature set.
....
