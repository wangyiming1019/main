...
[NOTE]
If an edit command only contains fields for private commands, the command will fail with an error message. +
However, if there contains at least one field that is public, the command will succeed, but any private fields will remain unmodified.
