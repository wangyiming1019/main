...
The sort enhancment is facilitated by the `sortBy` method in UniquePersonList. It supports
...
Instead, a message `"Cannot perform a search on that person. Their name is private."` will be printed in the UI to inform the user that they are not allowed to search for a person possessing a private `Name`.
...
[NOTE]
If a private field is to be edited by `EditCommand`, `EditCommand` will not modify the data of that field, even though the command succeeds. +
